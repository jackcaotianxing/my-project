    ...
    <template slot="children" slot-scope="{ props: { direction, selectedKeys }, on: { itemSelect } }">
      <template v-if="direction === 'left'">
        <a-tree
            ...
            @check="
            (_, props) => {handleLeftChecked(_,props,[...selectedKeys, ...targetKeys],itemSelect);}
          "
        />
        <a-empty v-else>
      ...
      <template v-else-if="direction === 'right'">
        <a-tree
            ...
            :expandedKeys.sync="rightExpandedKey"
            block-node
            checkable
            default-expand-all
            :tree-data="rightTreeData"
            @check="(_, props) => {handleRightChecked(_,props,[...selectedKeys, ...targetKeys],itemSelect);}
          "
        />
        <a-empty v-else>
          <template #description>
...
export default{
  ...
  methods: {
    ...
    processEditData() {
      ...
      this.leftHalfCheckedKeys = this.leftCheckedAllKey.filter((item) =>
          this.leftCheckedKey.indexOf(item) === -1)
      this.leftTreeData = handleLeftTreeData(
          cloneDeep(this.treeData),
          this.leftCheckedKey
      )
